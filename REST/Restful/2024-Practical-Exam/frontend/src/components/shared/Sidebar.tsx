...
const Sidebar = () => {
  ...
  return (
    <aside className="relative flex flex-1 flex-col min-h-0 border-r  pt-0 bg-primary">
            ...
            <li className="pb-4">
              <Link href="/" className="font-bold flex items-center lg:ml-2.5">
                ...
                <Text className="self-center whitespace-nowrap text-xl">
                  <Text className="text-white text-2xl">E</Text>-Libra
                </Text>
              </Link>
            </li>
            {pageDirectories.map((route, idx) => (
                <Link key={idx} href={route.path} className={`${isActive(route.path) ? "bg-white border border-white border-dashed text-primary font-medium text-md" : ""} text-base text-gray-900 font-normal rounded-lg hover:text-black hover:bg-green-200 duration-300 flex items-center p-2 group`}>
                    {route.icon}
